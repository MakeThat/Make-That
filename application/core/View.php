...
class View
{
    ...
    // if the page already has a built in header use that
    public function renderNoHeader($filename, $data = null)
    {
        if ($data) {
            foreach ($data as $key => $value) {
                $this->{$key} = $value;
    ...
    // For passing Javascript Files Along in raw
    public function RenderScript($filename, $data = null)
    {
        if ($data) {
            foreach ($data as $key => $value) {
                $this->{$key} = $value;
        ...
        require Config::get('PATH_VIEW') . $filename ;
    }
