...
class AdminModel
{
    ...
    public static function UpdateMakerspaceThings($tools, $active, $userId)
    {
        ...
        // write the above info to the database
          ...
          $query = $database->prepare("UPDATE users SET ActiveSub = :ActiveSub, ToolQualif = :ToolQualif  WHERE user_id = :user_id LIMIT 1");
          $query->execute(array(
                  ':ActiveSub' => $activeVAL,
                  ':ToolQualif' => $tools,
                  ':user_id' => $userId
          ...
          if ($query->rowCount() == 1) {
              Session::add('feedback_positive', Text::get('FEEDBACK_MAKERSPACE_INFO_UPDATED'));
              return true;
          }
    }
